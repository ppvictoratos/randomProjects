    ...
    var body: some View {
        NavigationView {
            List {
                ...
                Section(header: Text("Part 4: Pete's Stuff")) {
                    NavigationLink(destination: Example15(), label: {
                        Text("Example 14 (making donut)")
                    })
                }

            }.navigationBarTitle("SwiftUI Labs")
        }
    }
}
...
struct Example15: View {
    ...
    //they should rotate and change scale
    ...
    struct TriangleShape: Shape {
        ...
        func path(in rect: CGRect) -> Path {
            ...
            path.addLine(to: CGPoint(x: center.x + 12, y: center.y + 5))
            path.addLine(to: CGPoint(x: center.x, y: center.y - 5))
            return path
        }
    }
